...
const Cursor = () => {
  ...
  return (
    <span className="w-12">
      {solid ? (
        <span className="w-full h-full dark:bg-white white:bg-black border dark:border-white border-black" />
      ) : (
        <span className="h-full bg-none border border-gray-300 dark:border-gray-500" />
      )}
    </span>
  );
};
...
function RacingPage() {
  const [sampleText, setSampleText] = useState(
    () => sample(databasePrompts).paragraph
  );
      ...
      <div className="w-96 text-left">
        ...
        <div className="flex items-center justify-between my-3">
          <div className="flex items-center gap-3">
            <Button onClick={resetTest} disabled={testState !== TestState.DONE}>
              Reset
            </Button>
            <p className="text-gray-400 italic">
              {testState}
              {testState === TestState.NOT_STARTED && ". Type to start"}
            </p>
          </div>
          {(testState === TestState.RUNNING ||
            testState === TestState.DONE) && <p>{wpm} wpm</p>}
        </div>
        <div className="my-2">
          <p>
            <span className="dark:text-yellow-300 text-green-700">
              {correctText}
            </span>
            <span className="bg-red-500 opacity-30">{wrongText}</span>
            <Cursor />
            {remainderText}
          </p>
        </div>
